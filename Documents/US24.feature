...
Feature: US_024 - Toplantı Yönetim Sayfası
  ...
  @US24_TC01
  Scenario: Toplantılar sekmesi ve altında bulunan linklerinin Dashboard kenar çubuğunda görünür olması
  ...
  @US24_TC02
  Scenario: Rezervasyonlarım ikonuna tıklandığında toplantı istatistiklerinin görüntülenmesi
  ...
  @US24_TC03
  Scenario Outline: Filtrelenen toplantılar üzerinde CRUD işlemleri yapılabilmesi
    Given kullanıcı Toplantılar başlığı altında bulunan Rezervasyonlarım ikonuna tıklar
  ...
  @US24_TC04
  Scenario: Yalnızca açık toplantıları göster onay kutusunun çalışması (Rezervasyonlarım)
  ...
  @US24_TC05
  Scenario: Talepler linkine tıklandığında toplantı istatistiklerinin görüntülenmesi
    When kullanıcı Taleplerim linkine tıklar
    Then Taleplerim sayfası açılmalıdır
    And toplantı istatistik bilgileri sayfada görünür olmalıdır

  @US24_TC06
  Scenario: Toplantı üzerinde Add to Calendar işleminin gerçekleştirilebilmesi
  ...
  @US24_TC07
  Scenario: Toplantı üzerinde Contact Instructor işleminin gerçekleştirilebilmesi
    Given kullanıcı Taleplerim sayfasındadır
    And toplantı talep listesi görüntülenmektedir
    When kullanıcı bir toplantı seçer
  ...
  @US24_TC08
  Scenario: Toplantı üzerinde Finish Meeting işleminin gerçekleştirilebilmesi
    Given kullanıcı Taleplerim sayfasındadır
    And toplantı talep listesi görüntülenmektedir
    When kullanıcı bir toplantı seçer
  ...
  @US24_TC09
  Scenario: Yalnızca açık toplantıları göster onay kutusunun çalışması (Taleplerim)
  ...
  @US24_TC10
  Scenario: Ayarlar sayfasında gün bazlı zaman ekleme yapılabilmesi
  ...
  @US24_TC11
  Scenario: Yüz yüze toplantılarda saatlik ücret seçiminde min-max katılımcı sayısı belirlenebilmesi
  ...
  @US24_TC12
  Scenario: Grup toplantılarında saatlik ücret seçiminde min-max katılımcı sayısı belirlenebilmesi
    Given kullanıcı Ayarlar sayfasındadır
    When kullanıcı grup toplantısı için saatlik ücret seçim alanına gider
    Then minimum katılımcı sayısı giriş alanı görünür olmalıdır
    And maksimum katılımcı sayısı giriş alanı görünür olmalıdır
  ...
  @US24_TC13
  Scenario: Ayarlar sayfasında Kaydet butonunun görünür, aktif ve işlevsel olması
